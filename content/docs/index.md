...
Pi 是一个精简的终端编码工具框架。其核心力求保持小巧，同时可通过 TypeScript 扩展、技能、提示词模板、主题和 pi 包进行扩展。
...
对于通过 curl 和 npm 完成的安装，请使用 npm 卸载 pi 本身：
...
对于订阅提供商，请使用 `/login` 进行身份验证；或者在启动 pi 前设置 API 密钥，例如 `ANTHROPIC_API_KEY`。
...
- [会话](sessions.md) - 会话管理、分支和树状导航。
- [压缩](compaction.md) - 上下文压缩和分支摘要。

## 自定义

- [扩展](extensions.md) - 用于工具、命令、事件和自定义 UI 的 TypeScript 模块。
- [技能](skills.md) - 用于可复用按需能力的 Agent Skills。
- [提示词模板](prompt-templates.md) - 可从斜杠命令展开的可复用提示词。
- [主题](themes.md) - 内置及自定义终端主题。
- [Pi 包](packages.md) - 将扩展、技能、提示词和主题打包并共享。
...
- [JSON 事件流模式](json.md) - 提供结构化事件的打印模式。
